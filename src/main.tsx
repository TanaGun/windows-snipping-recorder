...
type NativeRecording = { path: string }
...
function App() {
  ...
  const toggleRecording = async () => {
    if (!isTauri()) { notify('Recording requires the Windows desktop build'); return }
    try {
      if (recording) {
        const result = await invoke<NativeRecording>('stop_screen_recording')
        setRecording(false)
        notify(`Saved MP4 locally: ${result.path}`)
      } else {
        await invoke<NativeRecording>('start_screen_recording')
        setSeconds(0)
        setRecording(true)
        notify('Recording screen locally — audio is not available yet')
      }
    } catch (error) {
      setRecording(false)
      notify(`Recording failed: ${String(error)}`)
    }
  }
  const saveNativeResult = (result: NativeCapture) => {
    setNativeCapture(result)
    setPreviewUrl(convertFileSrc(result.path))
